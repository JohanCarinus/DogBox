...
class DogsRepositoryImpl constructor(val dogApi: DogApi) : DogsRepository {
    ...
    override suspend fun getDogUrls(breed: String): List<UriImageData> {
        val stringUriList = dogApi.getRandomImages(PAGE_SIZE).message
        val uriList = mutableListOf<UriImageData>()
        for (stringUri in stringUriList) {
            uriList.add(UriImageData(Uri.parse(stringUri), R.drawable.placeholder))
        }
